...
class ListOrdersView(APIView):
    ...
    def get(self, request, format=None):
        ...
        try:
            ...
            return Response(
                {'error': 'Algo salió mal mostrando las órdenes.'},
    ...
    def get(self, request, transactionId, format=None):
        ...
        try:
            if Order.objects.filter(user=user, transaction_id=transactionId).exists():
                ...
                result['state_province_region'] = order.state_provinge_region
                ...
                return Response(
                    {'error': 'No existe una orden con ese ID.'},
                    status=status.HTTP_404_NOT_FOUND)
        except:
            return Response(
                {'error': 'Algo salió mal mostrando la orden.'},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR)