...
class GenerateTokenView(APIView):
    ...
    def get(self, request, format=None):
        ...
        try:
            ...
            return Response(
                {'error': 'Algo salió mal intentando obtener el token. Intenta de nuevo.'},
...
class GetPaymentTotalView(APIView):
    ...
    def get(self, request, format=None):
        ...
        coupon_name = request.qery_params.get('coupon_name')
        ...
        try:
            ...
            for cart_item in cart_items:
                ...
                if int(cart_item.count) > int(cart_item.product.quantity).exists():
                    return Response(
                ...
                shipping_cost = 0.0
                # verificar que el envío sea válido
            ...
            return Response(
                {'error': 'Algo salió mal obteniendo la información de pago. Intenta de nuevo.'},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR)
...
class ProcessPaymentView(APIView):
    ...
    def post(self, request, format=None):
        ...
        if not Shipping.objects.filter(id__iexact=shipping_id).exists():
            return Response(
                {'error': 'Opción de envío inválida.'},
                status=status.HTTP_404_NOT_FOUND)
        ...
        for cart_item in cart_items:
            ...
            if not Product.objects.filter(id=cart_item.product.id).exists():
                return Response(
                    {'error': 'Transacción fallida. No existe producto con ese ID.'},
                    status=status.HTTP_404_NOT_FOUND)
            if int(cart_item.count) > int(cart_item.prduct.quantity).exists():
                return Response(
                    {'error': 'No hay suficientes productos en stock.'},
                    status=status.HTTP_200_OK)
        ...
        if newTransaction.is_success or newTransaction.transaction:
            for cart_item in cart_items:
                ...
                # Obtener cantidad después de compra
            ...
            try:
                ...
                return Response(
                    {'error': 'La transacción fue exitosa pero falló al crear la orden.'},
                    status=status.HTTP_500_INTERNAL_SERVER_ERROR)
            ...
            for cart_item in cart_items:
                ...
                try:
                    ...
                    return Response(
                        {'error': 'Transacción exitosa y orden creada, pero falló al cargar los productos.'},
                        status=status.HTTP_500_INTERNAL_SERVER_ERROR)
            ...
            try:
                send_mail(
                    ...
                    + '\n\nActualmente está en proceso de preparación para enviarlo a su domicilio.'
                    + '\n\nPuedes ver el estado de tu orden en el perfil de su cuenta.'
                    + '\n\nMuchas gracias por su compra!'
                    + '\n\nEl equipo de Corralon Martinolich',
                ...
                return Response(
                    {'error': 'Transacción exitosa y orden creada, pero falló al emviar email.'},
                    status=status.HTTP_500_INTERNAL_SERVER_ERROR)
            ...
            try:
                ...
                return Response(
                    {'error': 'Error intentando actualizar el carrito de compras. Intente de nuevo'},
                    status=status.HTTP_500_INTERNAL_SERVER_ERROR)
            ...
            return Response(
                {'error': 'La transacción ha fallido. Intente de nuevo.'},
